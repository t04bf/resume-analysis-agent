...
- HR 摘要
- 面试建议
- 输入质量评分（inputQualityScore）
- 真实性边界说明（authenticityNotice）

## 七、如何保证“尽量真实”（务必阅读）

这个项目是 **AI 辅助筛选工具**，不是“自动录用器”。为了让结果更真实可用，代码里做了两层控制：

1. **输入质量评分**：根据简历/JD 文本长度和关键信息完整度给出 `inputQualityScore`（0-100）。
   - 文本越完整，评分越高，分析结果通常越稳定。
   - 如果评分偏低，建议让候选人补充项目细节后再分析。
2. **真实性提示**：每次响应都返回 `authenticityNotice`，明确提醒必须通过面试追问、背调等人工流程核验关键经历。

> 建议在业务侧设置阈值：
> - `inputQualityScore < 60`：标记为“需要补充材料”
> - `60 ~ 80`：可进入初筛，但需要重点追问
> - `> 80`：可作为较可靠的面试准备输入（仍需人工核验）
